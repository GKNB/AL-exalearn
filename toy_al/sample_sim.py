...
def _get_test_output(args):
    test_output = np.empty([0, 4])
    fname = args.sample_input_filename
...
def main():
    ...
    if args.phase == 0:
        ...
        test_output = _get_test_output("test_output") 
#        test_output = _get_test_output(test_output_filename_prefix) 
        print("test_output has shape {}".format(test_output.shape))
        print(test_output)
        sample_train = _sample_grid_base_on_test(test_output, num_train_sample, sigma, rng)
    ...
    _save_training_input(val_train, sample_train, val_test, sample_test, "training_input_phase_{}.hdf5".format(phase))
